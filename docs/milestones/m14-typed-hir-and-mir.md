...
- 新的 MIR 仅覆盖 M2 至 M13 已实现的语义，并以基本块、局部位置、指令与终结指令表达控制流和求值顺序；不得遗留 `PendingControlFlow`、原始 HIR `Expression` 或需要运行时名称查找的字符串引用。
- MIR 值、局部、指令和终结指令均保留 Yan `Type` 与 `Span`；执行失败仍可映射到稳定 Yan 诊断。
- `yan-mir` 提供无副作用的 MIR 验证入口，检查块与跳转目标、局部定义/使用、终结指令完整性、调用目标和类型一致性；解释器和未来后端均以通过验证的 MIR 为输入。
- `yan-eval` 改为只执行 MIR；不得重新进行名字查找或类型判断。
- M2 至 M13 的每个语义类别均有 Typed HIR 结构、MIR 结构和 MIR 执行输出三层断言。
...
5. 实现 MIR 验证器，并将解释器迁移至通过验证的 MIR。
...
- 不引入 SSA、常量折叠、死代码消除或其他优化 pass；性能优化必须由基准和目标后端的明确需求单独立项。
