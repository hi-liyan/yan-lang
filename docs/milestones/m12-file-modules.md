...
- `import module.path.Symbol` 只导入一个 `public` 的函数、struct、enum 或 newtype；同一模块的多个符号必须分别显式导入。
- `public` 仅允许修饰顶层函数、struct、enum 与 newtype。未标记的声明只能在当前模块使用。
...
- 导入不存在模块、不存在符号或非 `public` 符号时产生可定位的英文诊断。
...
- 不支持 `public` 字段、方法、局部变量或 enum variant，也不在本阶段增加依赖方向与 capability 检查。
