...
Add `yan-rust-backend` as a `yanc` dependency. Compute a deterministic hash from canonical entry path and source text using a standard-library hasher encoded as lowercase hex. Replace only `target/yan/<hash>/cargo` after generation succeeds; create a compiler-owned Cargo cwd and `CARGO_HOME` outside the current user Profile (on Windows, reject a relative `%PUBLIC%` and canonicalize it before appending `yanc`; elsewhere use the system temporary directory), then use `std::process::Command::new("cargo")` with fixed `build --quiet --manifest-path <absolute generated/Cargo.toml>`. Create the per-build cwd exclusively; before creating it and again after canonicalizing it, scan the cwd and every ancestor for `.cargo/config.toml` or legacy `config`, and inspect `CARGO_HOME/config.toml` and legacy `config` directly. Remove user `CARGO_BUILD_*`, `CARGO_TARGET_<TRIPLE>_*`, Rust wrapper and rustflags environment settings while retaining the system toolchain environment required by MSVC. Reject the build if configuration is found instead of accepting it from outside the Yan workspace. This detects pre-existing configuration but cannot provide a host-level guarantee against same-privilege filesystem changes after the checks. On success copy the compiled binary to `target/yan/<hash>/bin/<entry-name>[.exe]` and print the specified success line. On nonzero status or process start failure render `backend build failed` at the entry `1:1`; never forward child stderr.
