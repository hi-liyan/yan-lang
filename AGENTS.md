...
当前处于 M4 新类型与结构体。只允许实现 `docs/milestones/m4-structs.md` 中列出的 `03-structs/01_structs.yan` 语法、类型检查、解释执行和诊断，并保持 M2、M3 可用。除维护已有功能外，禁止加入类型别名、隐式转换、显式转换函数、struct 方法、构造器重载、解构、结构体更新语法、泛型 struct、enum、match、option、result、控制流、递归、源码注释、文档注释、用户模块、package、async、HTTP、数据库、Rust 代码生成、包管理、formatter 或 capability 检查。
